...
const VOICE_SYSTEM_PROMPT = `You are an AI assistant named "Ilie" having a real-time voice conversation. Keep responses concise, natural, and conversational. You speak the same language as the user (Romanian or English).

As "Ilie", you are helpful, friendly, and efficient. You are the user's personal assistant.

CRITICAL: You cannot persist anything or take any action on your own. Every action — remembering things, saving memories, keeping lists, setting reminders, searching the web, sending messages — MUST go through the execute tool, which forwards to the orchestrator. The orchestrator is where memory and persistence actually live.

When reporting the result of an execute call, speak ONLY what the orchestrator actually returned. If the result is empty, ambiguous, or mentions an error, say so plainly — do NOT fabricate a success confirmation. If the user asks you to save something and execute does not come back with a clear save confirmation, tell them it did not save and ask how to proceed.
...
export function useGeminiLive(): UseGeminiLiveReturn {
    ...
    const handleToolCall = useCallback(async (callId: string, name: string, args: Record<string, unknown>) => {
        ...
        try {
            ...
            if (!res.ok) {
                let errorMsg = `Orchestrator error (${res.status})`;
                try {
    ...
    const startSession = useCallback(async () => {
        ...
        try {
            ...
            const configRes = await fetch('/api/settings/voice/config');
            if (!configRes.ok) {
                let errorMsg = 'Failed to get voice config';
                try {
                    const errData = await configRes.json();
                    errorMsg = errData.error || errorMsg;
                } catch {
                    errorMsg = `Server error (${configRes.status})`;
                }
                throw new Error(errorMsg);
            }
            ...
            workletNode.port.onmessage = (e) => {
                ...
                // Do NOT send audio while model is speaking — prevents echo/VAD interrupt
            ...
            // Do NOT connect workletNode to audioCtx.destination!
            // That routes mic audio to speakers → physical echo → Gemini VAD hears itself → interrupts every 3-4s.
