...
import { MessageCircle, BarChart2, Bell, Timer, Box, Brain } from 'lucide-react';
    ...
    <div className="h-full flex flex-col space-y-6 max-w-2xl mx-auto pb-10">
      <div className="flex items-center justify-between">
        ...
        <div className="flex md:hidden items-center space-x-5 text-gray-500 dark:text-gray-400 mt-1 pr-1">
          <Link href="/orchestrator" className="hover:text-gray-900 dark:hover:text-gray-100 transition-colors p-1" aria-label="Commander">
            <Brain size={22} />
          </Link>
      ...
      <div className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
          Orchestratorul folosește SearXNG pentru căutări pe internet. Implicit rulează pe instanța publică <code className="bg-gray-100 dark:bg-zinc-800 px-1 rounded text-xs">searx.be</code>. Dacă ai SearXNG self-hosted, pune URL-ul tău.
        </p>
        ...
        <div className="flex gap-3">
          ...
          />
          <button
            onClick={handleSearxngSave}
            disabled={searxngStatus === 'Saving...'}
        ...
        {!searxngIsDefault && (
          <button
            onClick={() => { setSearxngUrl(''); handleSearxngSave(); }}
        ...
        {searxngStatus && searxngStatus !== 'Saving...' && (
          <p className={`text-sm font-medium mt-2 ${searxngStatus.includes('Error') ? 'text-red-500' : 'text-green-500'}`}>
            {searxngStatus}
          </p>
        )}
      </div>
            ...
            <button
              onClick={async () => {
                if (window.confirm('Are you sure you want to clear the agent memory? This will delete all conversation context.')) {
                  ...
                  try {
                    await api.post('/api/agent/memory/clear');
...
function KnowledgeBaseSection() {
  ...
  const loadEntries = useCallback(async () => {
    try {
      const data = await api.get<KBEntry[]>('/api/knowledge');
...
function GlassesGatewaySection() {
  ...
  useEffect(() => {
    api.get<{ hasToken?: boolean; token?: string }>('/api/settings/glasses-token')
      .then(data => {
    ...
    api.get<Memory[]>('/api/glasses/memories?limit=20')
  ...
  const handleClearMemories = async () => {
    ...
    await api.delete('/api/glasses/memories');
    setMemories([]);
  };
