...
export default function OrchestratorPage() {
    ...
    const loadHistory = useCallback(async () => {
        try {
            const data = await api.get<Message[]>('/api/orchestrator/chat/history');
    ...
    const handleSend = async () => {
        ...
        try {
            const data = await api.post<OrchestratorResponse>('/api/orchestrator/chat', { message: userMsg });
            ...
            } else if (data.booking && data.booking.success === false && data.booking.error) {
    ...
    const handleClear = async () => {
        ...
        await api.delete('/api/orchestrator/chat/history');
        setMessages([]);
            ...
            <div className="flex items-center justify-between pb-3 md:pb-4 border-b border-gray-200 dark:border-zinc-800">
                ...
                <div className="flex items-center gap-1">
                    <a 
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-500/10 text-amber-600 dark:text-amber-400 hover:bg-amber-500/20 rounded-lg transition-all text-[11px] font-bold border border-amber-500/20"
                        download
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/></svg>
                        APK
                    </a>
