...
export default function PlannerPage() {
    ...
    const loadHistory = useCallback(async () => {
        try {
            const data = await api.get<Message[]>('/api/planner/chat/history');
    ...
    useEffect(() => { loadHistory(); loadTasks(); }, [loadHistory, loadTasks]);
    ...
    const handleSend = async () => {
        ...
        try {
            const data = await api.post<ChatResponse>('/api/planner/chat', { message: userMsg });
    ...
    const handleClear = async () => {
        ...
        await api.delete('/api/planner/chat/history');
        setMessages([]);
    };
        ...
        <div className="h-[calc(100dvh-6rem)] md:h-[calc(100vh-4rem)] flex flex-col max-w-5xl mx-auto">
            ...
            <div className="flex items-center justify-between pb-2 border-b border-gray-200 dark:border-zinc-800 sticky top-0 bg-white dark:bg-[#0a0a0a] z-10">
                ...
                <div className="flex items-center gap-2">
                    <Link
                        href="/calendar"
                        className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg bg-blue-50 hover:bg-blue-100 dark:bg-blue-900/20 dark:hover:bg-blue-900/40 text-blue-600 dark:text-blue-400 font-medium transition-colors"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M8 2v4"/><path d="M16 2v4"/><rect width="18" height="18" x="3" y="4" rx="2"/><path d="M3 10h18"/></svg>
                        Calendar
                    </Link>
