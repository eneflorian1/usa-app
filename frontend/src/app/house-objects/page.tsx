...
export default function HouseObjectsPage() {
    ...
    const loadData = useCallback(async () => {
        try {
            const [objRes, dispRes, batchRes, toggleRes] = await Promise.all([
                fetch('/api/house-objects'),
                fetch('/api/displaced-objects'),
                fetch('/api/cleanup-batches/pending'),
                fetch('/api/settings/object-tracking')
            ]);
    ...
    const handleToggle = async () => {
        ...
        await fetch('/api/settings/object-tracking', {
            method: 'POST',
    ...
    const handleAddObject = async () => {
        ...
        try {
            const res = await fetch('/api/house-objects', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    const handleDeleteObject = async (id: string) => {
        ...
        await fetch(`/api/house-objects/${id}`, { method: 'DELETE' });
        loadData();
    };

    const handleApproveBatch = async (batchId: string) => {
        try {
            await fetch(`/api/cleanup-batches/${batchId}/approve`, { method: 'POST' });
    ...
    const handleDismissBatch = async (batchId: string) => {
        try {
            await fetch(`/api/cleanup-batches/${batchId}/dismiss`, { method: 'POST' });
            setPendingBatch(null);
            loadData();
        } catch (err) {
    ...
    const handleResolveDisplaced = async (id: string) => {
        await fetch(`/api/displaced-objects/${id}/resolve`, { method: 'PATCH' });
        loadData();
    };
