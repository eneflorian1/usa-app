...
export default function UgcProductPage() {
    ...
    const loadProducts = async () => {
        try {
            const res = await fetch('/api/ugc-product');
    ...
    const loadProductData = async (id: string, showLoader = true) => {
        if (showLoader) setLoading(true);
        try {
            const res = await fetch(`/api/ugc-product/${id}`);
            const data = await res.json();
            if (data._id) {
                setProduct(data);
    ...
    const deleteProduct = async (id: string) => {
        ...
        try {
            await fetch(`/api/ugc-product/${id}`, { method: 'DELETE' });
    ...
    const saveSystemPrompt = async () => {
        if (!product) return;
        await fetch(`/api/ugc-product/${product._id}/system-prompt`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
    ...
    const generateVisualBible = async () => {
        ...
        try {
            await fetch(`/api/ugc-product/${product._id}/visual-bible`, { method: 'POST' });
            await loadProductData(product._id, false);
            setActiveTab('bible');
        } catch (err) {
            console.error(err);
    ...
    const deleteGeneration = async (genId: string) => {
        if (!product) return;
        await fetch(`/api/ugc-product/${product._id}/generations/${genId}`, { method: 'DELETE' });
        loadProductData(product._id, false);
    };
