...
export default function UgcVideoPage() {
    ...
    const loadReferences = async () => {
        try {
            const res = await fetch('/api/ugc/references');
    ...
    const loadGenerations = async () => {
        try {
            const res = await fetch('/api/ugc-video/generations');
            const data = await res.json();
    ...
    const uploadFiles = async (files: FileList | File[]) => {
        ...
        for (let i = 0; i < imageFiles.length; i++) {
            ...
            try {
                ...
                await fetch('/api/ugc/references', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ name, imageData: base64, mimeType: file.type || 'image/jpeg' }),
                });
            } catch (err) {
    ...
    const deleteReference = async (id: string) => {
        await fetch(`/api/ugc/references/${id}`, { method: 'DELETE' });
    ...
    const pollGeneration = (genId: string, statusMsgId: string) => {
        ...
        pollingRef.current = setInterval(async () => {
            try {
                const res = await fetch(`/api/ugc-video/generations/${genId}`);
    ...
    const handleSend = async () => {
        ...
        try {
            const res = await fetch('/api/ugc-video/generate', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    prompt: userPrompt,
            ...
            updateMessage(statusId, {
                content: '❌ Eroare de conexiune cu serverul.',
                type: 'error',
            });
            setGenerating(false);
        }
    ...
    const deleteGeneration = async (id: string) => {
        await fetch(`/api/ugc-video/generations/${id}`, { method: 'DELETE' });
        loadGenerations();
    };
