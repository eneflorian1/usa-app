                    ...
                    <div className="flex items-center justify-between">
                        ...
                        {hasBookings && (
                            <span className="flex gap-0.5">
                                {dayBookings.slice(0, 3).map((_, idx) => (<span key={idx} className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-emerald-500 inline-block" />))}
                            </span>
                        )}
                    </div>
                    <div className="hidden md:block mt-1 space-y-0.5">
                        {dayBookings.slice(0, 2).map((b) => (
                            <div key={b._id} className="text-[10px] bg-emerald-100 dark:bg-emerald-900/40 text-emerald-800 dark:text-emerald-300 rounded px-1 py-0.5 truncate font-medium">{b.guestName}</div>
                        ))}
                        {dayBookings.length > 2 && <div className="text-[10px] text-gray-400">+{dayBookings.length - 2} more</div>}
                    </div>
                </div>
            );
        }
        ...
        <div className="h-full flex flex-col gap-4 md:gap-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                    <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 inline-block" /><span>Booked</span>
                </div>
            </div>
                ...
                <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-lg border border-gray-200 dark:border-zinc-800 overflow-hidden">
                    ...
                    <div className="p-4 md:p-6">
                        {selectedBookings.length === 0 ? (
                            <p className="text-sm text-gray-500 dark:text-gray-400 text-center py-4">No reservations for this day.</p>
                        ) : (
                            <div className="space-y-3">
                                {selectedBookings.map((b) => (
                                    <div key={b._id} className="flex items-start justify-between bg-gray-50 dark:bg-zinc-800/50 rounded-xl p-3 md:p-4">
                                        ...
                                        <button onClick={() => handleDeleteBooking(b._id)} className="ml-2 text-red-400 hover:text-red-600 transition-colors p-1 flex-shrink-0" title="Cancel booking">
