...
export default function AgentPage() {
  ...
  const loadHistory = useCallback(async () => {
    try {
      const data = await api.get<Message[]>('/api/agent/chat/history');
  ...
  const handleSend = async () => {
    ...
    try {
      const data = await api.post<ChatResponse>('/api/agent/chat', { message: userMsg });
      ...
      if (data.booking) {
        ...
        } else if (!data.booking.success && data.booking.error) {
          setMessages(prev => [...prev, {
            role: 'model',
  ...
  const handleClearHistory = async () => {
    ...
    try {
      await api.delete('/api/agent/chat/history');
      setMessages([]);
    } catch (err) {
