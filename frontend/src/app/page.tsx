...
export default function Home() {
  ...
  useEffect(() => {
    const now = new Date();
    api.get<Booking[]>(`/api/bookings/${now.getFullYear()}/${now.getMonth() + 1}`)
      .then(d => { if (Array.isArray(d)) setBookings(d); }).catch(() => {});
    api.get<Task[]>('/api/planner/tasks')
      .then(d => { if (Array.isArray(d)) setTasks(d); }).catch(() => {});
    api.get<Escalation[]>('/api/escalations')
      .then(d => { if (Array.isArray(d)) setEscalations(d); }).catch(() => {});
  }, []);
