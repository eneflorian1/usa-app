...
export default function TerminalPage() {
    ...
    const loadCommands = async () => {
        try {
            const res = await fetch('/api/local-exec/history');
    ...
    const sendCommand = async () => {
        ...
        setSending(true);
        try {
            await fetch('/api/local-exec/queue', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ command: manualCmd.trim(), label: 'Manual', cwd: manualCwd.trim() || '' }),
            });
            setManualCmd('');
