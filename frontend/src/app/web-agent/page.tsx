...
export default function WebAgentPage() {
    ...
    const loadSessions = useCallback(async () => {
        try {
            const data = await api.get<WebAgentSession[]>('/api/web-agent/sessions');
    ...
    useEffect(() => { loadSessions(); return () => { if (pollRef.current) clearInterval(pollRef.current); }; }, [loadSessions]);

    const startTask = async () => {
        if (!taskInput.trim() || launching) return;
        setLaunching(true);
        try {
            const data = await api.post<{ sessionId?: string }>('/api/web-agent/run', { task: taskInput.trim(), startUrl: urlInput.trim() || undefined });
            if (data.sessionId) {
                setTaskInput('');
                setUrlInput('');
    ...
    const pollSession = (sessionId: string) => {
        ...
        const fetchSession = async () => {
            try {
                const data = await api.get<WebAgentSession>(`/api/web-agent/sessions/${sessionId}`);
    ...
    const stopSession = async (id: string) => {
        try {
            await api.post(`/api/web-agent/sessions/${id}/stop`);
            loadSessions();
