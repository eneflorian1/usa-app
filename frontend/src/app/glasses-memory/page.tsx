...
export default function GlassesMemoryPage() {
    ...
    const loadMemories = useCallback(async () => {
        try {
            const data = await api.get<Memory[]>('/api/glasses/memories?limit=100');
    ...
    const handleDelete = async (id: string) => {
        await api.delete(`/api/glasses/memories/${id}`);
    ...
    const handleSyncFromVPS = async () => {
        ...
        try {
            const data = await api.post<{ synced: number; skipped: number }>('/api/glasses/sync-from-vps', { vpsUrl: 'http://155.117.45.192:5000' });
            setSyncStatus(`✅ ${data.synced} new, ${data.skipped} duplicates`);
            loadMemories();
        } catch (err) {
            setSyncStatus(`❌ ${(err as Error).message || 'Connection failed'}`);
        }
