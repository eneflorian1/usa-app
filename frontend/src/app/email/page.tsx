...
export default function EmailPage() {
  ...
  const [, setHiddenMessageIds] = useState<Set<string>>(() => {
  ...
  const checkApiKey = useCallback(async () => {
    try {
      const data = await api.get<{ apiKey?: string }>('/api/settings/agentmail');
  ...
  const loadMessages = useCallback(async (inbox: EmailInbox) => {
    ...
    try {
      const data = await api.get<{ messages?: EmailMessage[] }>(`/api/email/inboxes/${inbox.inboxId}/messages`);
  ...
  const loadInboxes = useCallback(async () => {
    ...
    setLoading(true); setError('');
    try {
      const data = await api.get<EmailInbox[]>('/api/email/inboxes');
      setInboxes(data);
      if (data.length > 0) {
  ...
  const loadAgentConfig = useCallback(async (inboxId: string) => {
    try {
      const data = await api.get<AgentConfig>(`/api/email/agent/config/${inboxId}`);
      setAgentConfig(data);
  ...
  const loadAgentThreads = useCallback(async (inboxId: string) => {
    try {
      const data = await api.get<EmailThread[]>(`/api/email/agent/threads/${inboxId}`);
  ...
  const loadProducts = useCallback(async () => {
    try {
      const data = await api.get<ProductResume[]>('/api/email/products');
      setProducts(data);
    } catch { /* ignore */ }
  }, []);
  ...
  const saveApiKey = async () => {
    ...
    setLoading(true); setError('');
    try {
      await api.post('/api/settings/agentmail', { apiKey: apiKeyInput.trim() });
  ...
  const createInbox = async () => {
    setLoading(true);
    try {
      await api.post('/api/email/inboxes', { displayName: newInboxName || 'AI Agent' });
  ...
  const deleteInbox = async (inboxId: string) => {
    ...
    try {
      await api.delete(`/api/email/inboxes/${inboxId}`);
  ...
  const sendEmail = async () => {
    ...
    setLoading(true); setError('');
    try {
      await api.post(`/api/email/inboxes/${selectedInbox.inboxId}/send`, { to: composeTo, subject: composeSubject, text: composeBody });
  ...
  const toggleAgent = async () => {
    ...
    try {
      await api.post(`/api/email/agent/${action}/${selectedInbox.inboxId}`);
  ...
  const saveAgentConfig = async () => {
    ...
    setAgentSaving(true);
    try {
      await api.post(`/api/email/agent/config/${selectedInbox.inboxId}`, { persona: agentPersona, maxRepliesPerThread: agentMaxReplies });
      await loadAgentConfig(selectedInbox.inboxId);
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'Failed');
  ...
  const processUrl = async () => {
    ...
    try {
      await api.post('/api/email/products/process', { url: productUrl.trim(), inboxId: selectedInbox?.inboxId });
      setProductUrl('');
      await loadProducts();
    } catch (err: unknown) {
  ...
  const deleteProduct = async (id: string) => {
    try {
      await api.delete(`/api/email/products/${id}`);
      await loadProducts();
    } catch { setError('Failed to delete'); }
  };
            ...
            {messages.map(msg => {
              ...
              return (
                <div key={msg.messageId} onClick={async () => {
                    try {
                      const data = await api.get<EmailMessage>(`/api/email/inboxes/${selectedInbox!.inboxId}/messages/${msg.messageId}`);
                    ...
                    <div key={t._id} onClick={() => { setSelectedThread(t); setView('thread'); }}
                      ...
                      <div className="flex items-center gap-2 ml-2 shrink-0">
                        ...
                        <button onClick={async (e) => {
                            e.stopPropagation();
                            try {
                              await api.delete(`/api/email/agent/thread/${t._id}`);
