    ...
    return (
        <div ref={panelRef} className={isNotificationsPage ? 'hidden' : ''}>
            <div className="fixed z-50 bottom-20 md:bottom-6 right-4 md:right-6 flex flex-col items-end gap-3">
                ...
                {/* Dropdown panel */}
                {open && (
                    <div className="pointer-events-auto w-80 bg-zinc-950/95 border border-amber-500/30 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-200">
                        {/* Header */}
                ...
                >
                    ...
                    {/* Badge cu număr */}
                    {pendingCount > 0 && (
                        <span className="absolute -top-1 -right-1 min-w-[20px] h-5 px-1 flex items-center justify-center rounded-full bg-red-500 text-white text-[10px] font-bold border-2 border-black">
                            {pendingCount > 99 ? '99+' : pendingCount}
                        </span>
                    )}

                    {/* Pulse ring când sunt alerte pending */}
                    {pendingCount > 0 && !open && (
